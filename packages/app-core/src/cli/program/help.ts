...
const EXAMPLES = [
  ...
  ["eliza setup", "Initialize config file and the agent workspace."],
...
export function configureProgramHelp(program: Command, programVersion: string) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under a separate directory with its own config and ports",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates state dir and config under a profile-specific directory)",
    );
