...
<Warning>
Release `v2.0.0-alpha.116` does not ship an in-repo Chrome extension app, and the Browser Relay extension is not part of the shipped release surface for this repository checkout.
</Warning>

## Release status

The **Milady Browser Relay** remains a planned or separately distributed extension. This repository does not contain the extension source, an unpacked extension directory, or a supported in-repo installation path for release `v2.0.0-alpha.116`.
...
For release `v2.0.0-alpha.116`, treat browser control as one of these:
