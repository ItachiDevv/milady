...
4. Add **streaming plugins** for any configured streaming destinations (Twitch, YouTube, X, Custom RTMP, Pump.fun) using `STREAMING_PLUGINS`
5. Add **model-provider plugins** when their API key environment variables are detected (e.g., `ANTHROPIC_API_KEY` triggers `@elizaos/plugin-anthropic`)
6. Handle **Eliza Cloud mode** -- when cloud is active, remove direct AI provider plugins since the cloud plugin handles all model calls
7. Add plugins from **`config.plugins.entries`** with `enabled !== false`
8. Add plugins from **feature flags** (`config.features`)
9. Add **user-installed plugins** from `config.plugins.installs`
10. Enforce the **shell feature gate** -- remove `plugin-shell` if `config.features.shellEnabled === false`
