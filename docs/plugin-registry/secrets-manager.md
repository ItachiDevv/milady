...
**Package:** `@elizaos/plugin-secrets-manager` (statically imported and available at startup — not in the `CORE_PLUGINS` auto-load list, but loaded early via direct import in the runtime so secrets are available before other plugins initialize)
