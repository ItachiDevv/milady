...
Milady supports **23 model providers** out of the box. Providers are loaded as plugins and can be auto-enabled by setting the right environment variable, or explicitly configured in `milady.json`.
...
| [Pi AI](https://pi.ai) | `@elizaos/plugin-pi-ai` | `ELIZA_USE_PI_AI` or `MILADY_USE_PI_AI` | No | Inflection Pi conversational models. |
...
| CUA | `@elizaos/plugin-cua` | `CUA_API_KEY` or `CUA_HOST` | No | Cloud sandbox automation (Computer Use Agent). |
| Obsidian | `@elizaos/plugin-obsidian` | `OBSIDIAN_VAULT_PATH` | No | Obsidian vault integration. Auto-enables when vault path is set. |
| RepoPrompt | `@elizaos/plugin-repoprompt` | `REPOPROMPT_CLI_PATH` | No | RepoPrompt CLI integration for codebase context. |
| Claude Code Workbench | `@elizaos/plugin-claude-code-workbench` | `CLAUDE_CODE_WORKBENCH_ENABLED` | No | Claude Code workbench integration. |
...
| `MILADY_USE_PI_AI` | Pi AI |
| `ELIZAOS_CLOUD_API_KEY` | elizaOS Cloud |
| `ELIZAOS_CLOUD_ENABLED` | elizaOS Cloud |
| `CUA_API_KEY` | CUA |
| `CUA_HOST` | CUA |
| `OBSIDIAN_VAULT_PATH` | Obsidian |
| `REPOPROMPT_CLI_PATH` | RepoPrompt |
| `CLAUDE_CODE_WORKBENCH_ENABLED` | Claude Code Workbench |
