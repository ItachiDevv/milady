...
Milady supports **19 model providers** out of the box — 18 standard providers plus an elizaOS Cloud managed option. Providers are loaded as plugins and can be auto-enabled by setting the right environment variable, or explicitly configured in `milady.json`.
