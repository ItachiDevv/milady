...
<Note>
Additional env variables like `CUA_API_KEY`, `OBSIDIAN_VAULT_PATH`, `REPOPROMPT_CLI_PATH`, and `CLAUDE_CODE_WORKBENCH_ENABLED` also trigger plugin auto-enable, but these activate **feature plugins** rather than model providers. See [Configuration](/configuration) for the full list.
</Note>
