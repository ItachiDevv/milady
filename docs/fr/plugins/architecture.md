...
Les plugins tels que `@elizaos/plugin-directives`, `@elizaos/plugin-mcp` et `@elizaos/plugin-scheduling` sont commentés dans le code source et pourraient être activés dans de futures versions. Notez que `@elizaos/plugin-commands` est maintenant actif et inclus dans `CORE_PLUGINS`.
