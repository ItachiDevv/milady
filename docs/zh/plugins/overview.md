...
3. **核心插件** — 始终加载：`sql`、`local-embedding`、`form`、`knowledge`、`trajectory-logger`、`agent-orchestrator`、`cron`、`shell`、`agent-skills`、`commands`、`roles`（见 `packages/agent/src/runtime/core-plugins.ts`）。`pdf`、`browser`、`computeruse`、`obsidian`、`code`、`repoprompt`、`claude-code-workbench`、`vision`、`cli`、`edge-tts` 和 `elevenlabs` 等附加插件是可选的，当其功能标志或环境变量配置后才会加载。
