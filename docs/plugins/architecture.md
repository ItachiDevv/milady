...
> **Note:** `@elizaos/plugin-secrets-manager`, `@elizaos/plugin-rolodex`, `@elizaos/plugin-plugin-manager`, `@elizaos/plugin-trust`, `@elizaos/plugin-todo`, `@elizaos/plugin-personality`, and `@elizaos/plugin-experience` are statically imported for fast resolution but commented out of the core list. They may be re-enabled in a future release. `@miladyai/plugin-roles` is a Milady-specific core plugin that provides role-based access control (OWNER, ADMIN, NONE) with connector whitelisting.
...
Plugins such as `@elizaos/plugin-directives`, `@elizaos/plugin-mcp`, and `@elizaos/plugin-scheduling` are commented out in the source and may be activated in future releases. Note that `@elizaos/plugin-commands` is now active and included in `CORE_PLUGINS`.
