...
<Info>
Bluesky is **not bundled** with Milady. It is an elizaOS registry plugin that must be installed manually (`milady plugins install bluesky`) before use. It is not auto-enabled from connector config alone.
</Info>
