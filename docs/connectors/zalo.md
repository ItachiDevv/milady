...
The Zalo connector is an elizaOS plugin that bridges your agent to the Zalo platform via the Official Account API. A personal-account variant is also available as `@elizaos/plugin-zalouser`.

<Info>
This connector is **not bundled** with Milady. It is an elizaOS registry plugin that must be installed manually (`milady plugins install zalo`) before use.
</Info>
