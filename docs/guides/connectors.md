...
20 connectors are **auto-enabled** — just add their config block and Milady loads the plugin automatically. The remaining connectors (Bluesky, Instagram, LINE, Zalo, Twilio, GitHub, Gmail Watch, Nextcloud Talk, Tlon) must be installed from the registry first with `milady plugins install <name>`.
