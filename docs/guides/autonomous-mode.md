...
| **Position** | 10 |
| **Source** | Autonomous state provider (registered dynamically) |
...
| **Max active triggers** | 100 per creator (default) | Prevents runaway trigger creation. Configurable via `MILADY_TRIGGERS_MAX_ACTIVE` env var. |
