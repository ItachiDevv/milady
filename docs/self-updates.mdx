...
# Disable automatic update checks (edit milady.json directly)
# Set "update": { "checkOnStart": false } in ~/.milady/milady.json
```
...
Or inspect the current value:

```bash
milady config get update.checkOnStart
```
