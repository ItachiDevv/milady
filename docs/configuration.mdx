...
| `signal` | `@elizaos/plugin-signal` | `connectors.signal.authDir`, `account`, `httpUrl`, `httpHost`, `httpPort`, `cliPath` (any one), or `accounts` with enabled entries |
...
| `wechat` | `@miladyai/plugin-wechat` | `connectors.wechat.apiKey` or `accounts` with `apiKey` (Milady-specific) |
